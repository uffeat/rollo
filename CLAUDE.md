...
In general, the iworker is used by the client as an invisible iframe (height set 0; display none can trup up the Anvil app), but occasionally I do let the iframe become visible. This is especially relevant, if I wish to use some of Anvil's special built-in modals, e.g., re user management and payments.

Moreover, the server-app can also be used as a "side-by-side" app with respect to the client app, i.e., the server-app can also operate independently (both as an SPS and an MPA) that the client app can link to and to a limited degree correspond with. I therefor

### How they connect

The client app builds to a main sheet (containing all styles + encoded parcel
...
- Management of JS vs. Python types. Anvil's client-side Python is pretty good at handling JS stuff directly. That's mostly great, but can sometimes be deceptive. For example, you can destructure a JS vanilla object directly into a Python callable with the `**` syntax, and in many cases you can use a JS vanilla object as if it were a dict... But attempts to, e.g., call `get()` of a vanilla JS object blows things up. I use different strategies to deal with this, including:
  ...
  In general (but not 100% strictly) - to keep my sanity - I lean towards the paradigm: In the client side of the server app everything should be Python, i.e., if JS types comes in it should be converted to Python types or at least be able to be used as-if Python types.
- Anvil's client-side Python sometimes incorrectly deals with JS stuff and special adapters must be written. Fortunately, this is rare, but when it happens things blow up. Notably, certain JS proxies are not handled correctly, e.g., the central `component` feature in the rollo parcel and also the `use` feature itself. In the client app, I trive to avoid making compromises just because consumption in Anvil trips things up, but in some cases I do accomodate multiple ways of doing things with Anvil in mind.
...
My custom build tool also exposes `use` itself publically so that it can be consumed by the server-app's client code (and own assets).
...
- Parcels prefixed with `_` (e.g., `_accordion`) are INACTIVE — placeholders
  for future work or work in progress. Ignore them. 
...
While JS-first, I also use a VS Code plugin intended for Lit (but not used for Lit). I use this plugin to enable linted authoring of html in JS. This open up many possibilities for creative use of html.
...
When used in Anvil, we do some hacks to ensure that even Anvil components reside inside the app component.
...
- I want to make `frame` "less fundamental" or purely about layout, whereas `app` serves a more logic purpose.
- The `frame` component relies on Bootstrap. Hence, being "dirty" (not zero-dep) it does not belong in the rollo parcel, but has it's own. In principle, I wish to be open to special or future scenarions, where the rollo parcel can be used without the frame component.

When used in Anvil, we do some hacks to implement the frame component inside an Anvil component. When Anvil is used as an iframe, the frame component is typically used as an empty visual layer to provide visual consistency with respect to the client app. However, if Anvil is used as a stand-alone app, the frame component (or the Anvil component that wraps it) is used as an actual layout component.
...
- **Top-level `_` dirs** (`_history`, `_notes`, `_scratch`, `_stash`,
  `_reference`): personal workspace directories — not part of the app. In genral, ignore unless specifically asked.
- **`_` prefixed parcels** (e.g., `_accordion`, `_dropdown`): inactive placeholders for future work. They reflect an older structure and will be reshaped when activated. Ignore them.
