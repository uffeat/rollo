...
I had not experienced any issues with this version.
However, I asked Codex to check my inflight patterns and it came up with:

1. use.js public (/) source fetch path: when fetch(...).text() throws (offline/cors/etc.), 
  the fetching map entry never gets cleared because the code doesn’t catch/finalize around the fetch/trim. 
  Any subsequent request for the same path then awaits a forever-pending promise. 
  Wrap the fetch + text parse in try/catch/finally that always deletes the fetching entry 
  (and rejects/raises appropriately).

2. use.js JS transformer (use.types.add("js")): the constructing map adds an inflight promise 
but doesn’t remove it if Function(...) or owner.module(...) throws/rejects. 
After a single failure the path stays stuck and future calls keep awaiting the dead promise. 
Use try/finally to delete the map entry and reject so callers can retry.

3. use.js assets.get timeout branch: the custom Promise.withResolvers wrapper never settles 
if the source handler rejects (no catch), and the timeout timer isn’t cleared. That leaves an unresolved inflight and a dangling timer until timeout fires (or forever if timeout is falsy). Add .catch on the source promise to clear the timer and resolve/reject the outer promise so failures release inflight state.

I subsequently asked Codex to implement these issue. This version is the latest before these changes.
*/
