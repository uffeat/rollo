...
`, n = n.substring(u.length + 1), a = !0), !a) {
          let q = this.rules.other.nextBulletRegex(f), J = this.rules.other.hrRegex(f), K = this.rules.other.fencesBeginRegex(f), V = this.rules.other.headingBeginRegex(f), de = this.rules.other.htmlBeginRegex(f);
          for (; n; ) {
            let Z = n.split(`
`, 1)[0], S;
            if (u = Z, this.options.pedantic ? (u = u.replace(this.rules.other.listReplaceNesting, "  "), S = u) : S = u.replace(this.rules.other.tabCharGlobal, "    "), K.test(u) || V.test(u) || de.test(u) || q.test(u) || J.test(u)) break;
