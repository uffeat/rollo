import "../../use";
const { Mixins, author, component, css, mix, stateMixin } =
  await use("@/rollo/");
...
export const Input = author(
  class extends mix(HTMLElement, {}, ...Mixins(stateMixin)) {
    ...
    constructor() {
      ...
      const shadow = component.div({ id: "root" }, slot);
    ...
    __new__(...args) {
      ...
      this.tree.message = component.div(".form-text"); //
      this.tree.floating.append(
        this.tree.input,
        this.tree.label,
        this.tree.message,
      );
