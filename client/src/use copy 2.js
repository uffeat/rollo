...
export class Store {
  ...
  async get(key) {
    if (this.#_.registry.has(key)) {
      const value = this.#_.registry.get(key);
      if (value instanceof Promise) {
        const resolved = await value;
        this.#_.registry.set(key, resolved);
        return resolved;
      }
      return value;
    }
  ...
  setup(key) {
    UseError.if(this.#_.registry.has(key), `'setup' already used for key: ${key}`)
    const pwr = Promise.withResolvers();
    this.#_.registry.set(key, pwr.promise);
    return {
      resolve: (result) => {
        this.#_.registry.set(key, result);
        pwr.resolve(result);
        return result;
      },
      reject: (error) => {
...
(() => {
  ...
  use.sources.add("/", async ({ options, owner, path }) => {
    ...
    if (path.type === "css" && as === undefined && raw !== true) {
      ...
      //console.log("registry:", store.__registry__); ////
