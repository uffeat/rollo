...
export const Plot = author(
  class extends mix(HTMLElement, {}, ...Mixins(stateMixin)) {
    ...
    update({ config, data, layout, ...updates } = {}) {
      ...
      /* Add defaults to layout */
      if (layout == null) layout = {}; // only when missing
      //layout.colorway ??= colorway;
      //layout.font = { ...(layout.font ?? {}), color: css.root.bsLight };
      const L = (this.#_.layout = layout ?? this.#_.layout ?? {});
      L.colorway ??= colorway;
      L.font ??= {};
