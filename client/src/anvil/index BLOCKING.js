...
*/
export const Anvil = async (src) => {
  let iframe = document.getElementById("anvil");
  /* Guard against multiple calls (albeit not likely). */
  if (iframe) {
    /* Guard against the illusion that 'src' can be changed. */
    Exception.if(src !== iframe.detail.origin, `Cannot change origin.`);
  ...
  */
  ...
  /* Return "pseudo module". Avoids the need for a `if (key === 'then') return` 
