...
export const Use = async ({ env, origin, text = _text } = {}) => {
  //
  //
  console.log("Initializing import engine."); ////
  //
  //
  ...
  const assets = new (class Assets {
    ...
    async _setup() {
      ...
      this.#_.get = async (path, ...args) => {
        ...
        const { callback, process = true, raw = false } = options;

        /* Raw */
        if (raw) {
          const result = await this.text(path.path);
          callback?.(result, { options, path }, ...args);
          return result;
        }
        ...
        if (this.processors.has(path.types)) {
          ...
          if (processed !== undefined) {
            callback?.(processed, { options, path, result }, ...args);
            return processed;
          }
        }
        callback?.(result, { options, path }, ...args);
    ...
    NOTE Virtual asset: Path-specific asset added at runtime. 
    Useful during DEV for overwriting committed assets. 
    Operates on 'path.path'. */
    get vassets() {
