...
def main(use, *args, **kwargs):
    ...
    class assets:
        ...
        def __init__(self):
            ...
            class _type:
                ...
                def __call__(self, handler):
                    key = next(iter(self.args), handler.__name__.lower())
                    if isinstance(handler, type):
                        handler = (
                            handler(owner=owner)
                            if "__init__" in handler.__dict__
                            else handler()
                        )
                    types[key] = handler
                    return handler
        ...
        def __call__(self, path, test=False, **options):
            return self._["types"].get(path.type, self.get_text)(path, test=test, **options)



        @property
        def type(self):
            """Decorates type handler."""
            return self._["type"]
        ...
        def get_text(self, path, cache=False, test=False, **kwargs):
            if meta.DEV and test:
                try:
    ...
    class css:
        ...
        def __call__(self, path, test=False, **options):
            ...
            if options.get("raw"):
                return owner.get_text(path, test=test)

            # HACK Cannot easily inject link-based sheet -> use style tag
            if meta.DEV and test:
                try:
                    style = document.head.querySelector(f'style[path="{path.path}"]')
                    if style:
                        return style
    ...
    class html:
        ...
        def __call__(self, path, test=False, **options):
            ...
            if path.types == "css.html":
                ...
                node = owner.create_node(path, test=test)
            ...
            if path.types == "jinja.html":
                ...
                if not result:
                    raise KeyError(f"Invalid asset: {path.path}.")
                return owner.render(result, **options.get("data", {}))
    ...
    class jinja:
        ...
        def __call__(self, path, test=False, **options):
            ...
            # Ensure subsequent treatment as html
            path.type = path.types = "html"
            text = owner.get_text(path, test=test)
            return owner.render(text, **options.get("data", {}))
    ...
    class md:
        ...
        def __call__(self, path, test=False, **options):
            ...
            if path.path in self.cache:
                return self.cache[path.path]
            content = owner.get_text(path, test=test)
