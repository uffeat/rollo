...
class Base(works.HtmlTemplate):
    ...
    def setup(self):
        ...
        # XXX TODO Purge native options -> Auto and/or fire both 
        ...
        class on:
            ...
            def __call__(self, handler):
                event_name = self.kwargs.get("type")
                if not event_name:
                    if self.native:
                        event_name = handler.__name__
                    else:
                        event_name = handler.__name__.replace("_", "-")
                if self.native:
    ...
    @property
    def slot(self) -> str:
        return self._.get("slot", "default")
